void Read_Tree_make_plots_v1(){
    ...
    const int NbOfFilesToRead = 11;
    string filepath = "./";
    //string rootfilename[NbOfFilesToRead] ={"run8188","run8189"};
    string rootfilename[NbOfFilesToRead] ={"run9042","run9043","run9052","run9044","run9045","run9046","run9047","run9048","run9049","run9050","run9053"};
    string postfix = "_Tree.root";
    string outputrootfilename = "./run9042-9053-24ns.root";
    ...
    string sDigiCh[8] = {"S0","S1","S2","S3","S4","S5","",""};
    int TDCIndexMap[6] = {10,11,12,13,14,15};
    ...
    TH1F* hDigiRT[6][4];
    TH1F* hDigiPW[6][4];
    TH2F* hScatter_Charge_ST[6][4];
    TH1F* hDigiCharge_ThresholdTest[6];
    TH2F* hTDCTest = new TH2F("TDCtest","",18,0,18,5000,0,5000);
    ...
    for(int i=0;i<8;i++){
        ...
        for(int j=0;j<4;j++){
            ...
            if(i<6){
                ...
                hDigiCharge[i][j] = new TH1F(tempname,"",3000,0,3000);
                ...
                hDigiAmp[i][j] = new TH1F(tempname,"",1500,0,1500);
                ...
                sprintf(tempname,"hCharge_vs_PW_%s_%s",sDigiChMap[i],sTriggerType[j]);
                hScatter_Charge_ST[i][j] = new TH2F(tempname,"",1500,0,1500,310,-10,300);    
                hScatter_Charge_ST[i][j]->SetXTitle("ADC channel");
                hScatter_Charge_ST[i][j]->SetYTitle("Time bin");
                if(j==0){
                     sprintf(tempname,"hCharge_%s_LEDTrig_%sOnly",sDigiChMap[i],sDigiCh[i]);
                     hDigiCharge_ThresholdTest[i] = new TH1F(tempname,"",3000,0,3000);
                hDigiCharge_ThresholdTest[i]->SetXTitle("ADC channel");
                hDigiCharge_ThresholdTest[i]->SetYTitle("Counts");
                }
            }
        }
            ...
            for(int k=0;k<8;k++){
               ...
               if(k<6){
                  if( (evtDigiAmplitude[k]) > (5.0*evtDigiPS[k]) ){
                    ...
                    hScatter_Charge_ST[k][0]->Fill(evtDigiCharge[k],evtDigiPW[k]);
                    ...
                    hScatter_Charge_ST[k][evtTrigType]->Fill(evtDigiCharge[k],evtDigiPW[k]);
                  }
                  if(evtTrigType==3 && evtTDC[TDCIndexMap[k]]!=-1)
                  {
                        hDigiCharge_ThresholdTest[k]->Fill(evtDigiCharge[k]);
                  }
               }
    ...
    for(int i=0;i<8;i++){
        for(int j=0;j<4;j++){
            ...
            if(i<6){
                ...
                hDigiRT[i][j]->Write();
                hDigiPW[i][j]->Write();
                hScatter_Charge_ST[i][j]->Write();
                if(j==0){
                   hDigiCharge_ThresholdTest[i]->Write();
                }
            }
        }
    }
